...
namespace SPH
{
	...
	*/
    ...
    static Real W_with_h(const Vector3r &r, const Real h)
    {
      Real old_h = m_radius; 
      m_radius = h;
			const Real pi = static_cast<Real>(M_PI);
      ...
      m_radius = old_h;
			const Real old_h3 = m_radius*m_radius*m_radius;
			m_k = static_cast<Real>(8.0) / (pi*old_h3);
			m_l = static_cast<Real>(48.0) / (pi*old_h3);
      return result;
    }
    ...
    static Vector3r gradW_with_h(const Vector3r &r, const Real h)
    {
      Real old_h = m_radius; 
      m_radius = h;
			const Real pi = static_cast<Real>(M_PI);
    ...
    // Used in backward 
    static Matrix3r gradGradW(const Vector3r &r)
    {
        Matrix3r res;
        const Real rl = r.norm();
        const Real q = rl / m_radius; 
			...
			{
          const Vector3r gradq = r * (static_cast<Real>(1.0) / (rl*m_radius));
          const Matrix3r gradGradq = (static_cast<Real>(1.0) / (rl*m_radius)) * 
                  (Matrix3r::Identity() - r * r.transpose() / (rl*rl));

          if (q <= 0.5)
          {
            res = m_l*q*((Real) 3.0*q - static_cast<Real>(2.0))*gradGradq 
                + m_l*((Real)6.0 * q - 2) * gradq * gradq.transpose();
          }
          else
          {
            const Real factor = static_cast<Real>(1.0) - q;
            res = m_l*(-factor*factor)*gradGradq + m_l * (Real)2.0 * factor * gradq * gradq.transpose();
          }
			}
			else
				res.setZero();
    ...
    static Matrix3r gradGradW_with_h(const Vector3r &r, const Real h)
    {
      Real old_h = m_radius; 
      m_radius = h;
			const Real pi = static_cast<Real>(M_PI);
	...
	*/
    ...
    static Matrix3r gradGradW(const Vector3r &r)
    {
       throw "Not implemented gradGradW!";
	...
	*/
    ...
    static Vector3r gradW_with_h(const Vector3r &r, const Real h)
    {
      ...
      return result;
		...
		static Matrix3r gradGradW(const Vector3r& r)
		{
			const Real r1 = r.norm();
			const Real q = r1 / m_radius;
			Matrix3r res;
			if (q <= 1.0 && r1 > 1.0e-5)
			{
				const Vector3r gradq = r * (static_cast<Real>(1.0) / (r1 * m_radius));
				const Real ml_R2 = m_l / m_radius / m_radius;
				if (q <= 0.5)
				{
					res = ml_R2 * (3.0 * gradq * r.transpose() + (3.0 * q - 2.0) * Matrix3r::Identity());
				}
				else
				{
					res = -ml_R2 * ((1.0 - q) * (1.0 - q) / q * Matrix3r::Identity() + (1 - 1 / q / q) * gradq * r.transpose());
				}
			}
			else
				res.setZero();
