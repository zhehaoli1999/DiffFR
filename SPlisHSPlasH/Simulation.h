...
namespace SPH
{
	enum class SimulationMethods { WCSPH = 0, PCISPH, PBF, IISPH, DFSPH, DiffDFSPH, PF, ICSPH, NumSimulationMethods };
	enum class BoundaryHandlingMethods { Akinci2012 = 0, Koschier2017, Bender2019, NumSimulationMethods };
  enum class GradientMode{Complete=0, Incomplete, RigidGradOnly};
  enum class RigidBodyMode{WithGyroscopic=0, NoGyroscopic};
	...
	*/
	...
	public:
		static int SIM_2D;
		static int IS_DEBUG;
		static int USE_RIGID_CONTACT_SOLVER;
		static int USE_RIGID_GRADIENT_MANAGER;
		static int FRICTION_COEFF;
		static int BETA_SCALE_FACTOR;
		static int RR_CONTACT_GAMMA_FACTOR;
		static int RR_CONTACT_DENSITY_ERR_THRESH;
		static int RR_CONTACT_SOLVER_MAX_ITER;
		static int USE_RR_CONTACT_GRADIENT;
		static int IS_DEBUG_RIGID_CONTACT;
		static int IS_RELEASE_RIGID_BODY_MODE;
		...
		static int GRADIENT_MODE;
		static int RIGIDBODY_MODE;
    static int ENUM_RB_WITH_GYROSCOPIC;
    static int ENUM_RB_NO_GYROSCOPIC;

    static int ENUM_GRADIENT_COMPLETE;
    static int ENUM_GRADIENT_INCOMPLETE;
    static int ENUM_GRADIENT_RIGIDONLY;
		...
		static int ENUM_SIMULATION_DiffDFSPH;
	...
	protected:
		...
		Real(*m_kernelFct_with_h)(const Vector3r &r, const Real h);
		Vector3r(*m_gradKernelFct)(const Vector3r &r);
		Vector3r(*m_gradKernelFct_with_h)(const Vector3r &r, const Real h);
		Matrix3r(*m_gradGradKernelFct)(const Vector3r &r);
		Matrix3r(*m_gradGradKernelFct_with_h)(const Vector3r &r, const Real h);
		SimulationMethods m_simulationMethod;
		GradientMode m_gradientMode;
    RigidBodyMode m_rbMode;
		TimeStep *m_timeStep;
		...
		bool m_sim2D;
		bool m_enableZSort;
    bool m_useRigidContactSolver; 
    bool m_useRigidGradientManager; 
    bool m_releaseRigidBodyMode; 
		...
		bool m_isDebug;
		bool m_isDebugRigidContact;
    Real m_friction_coeff; // used in rigid-rigid contact solver
    Real m_beta_scale_factor; // used in rigid-rigid contact solver
    Real m_rigid_contact_gamma_factor; // used in rigid-rigid contact solver
    Real m_rigid_contact_density_err_thresh; // density err thresh 
    Real m_rigid_contact_support_radius_factor; // density err thresh 
    unsigned m_RR_contact_solver_max_iter; // used in rigid-rigid contact solver
    bool m_use_RR_contact_gradient;
#ifdef USE_DEBUG_TOOLS
	...
	public:
		...
		*/
		...
		static Simulation* getCurrentWithBase (SimulatorBase*);
		...
		FORCE_INLINE Real W_zero_with_h(const Real h) const { return m_kernelFct_with_h(Vector3r::Zero(), h); }
		FORCE_INLINE Real W(const Vector3r &r) const { return m_kernelFct(r); }
		FORCE_INLINE Real W_with_h(const Vector3r &r, const Real h) const { return m_kernelFct_with_h(r, h); }
		FORCE_INLINE Vector3r gradW(const Vector3r &r) { return m_gradKernelFct(r); }
		FORCE_INLINE Vector3r gradW_with_h(const Vector3r &r, const Real h) { return m_gradKernelFct_with_h(r, h); }
		FORCE_INLINE Matrix3r gradGradW(const Vector3r &r) { return m_gradGradKernelFct(r); }
		FORCE_INLINE Matrix3r gradGradW_with_h(const Vector3r &r, const Real h) { return m_gradGradKernelFct_with_h(r, h); }

		int getSimulationMethod() const { return static_cast<int>(m_simulationMethod); }
		void setSimulationMethod(const int val);

		int getGradientMode() const { return static_cast<int>(m_gradientMode); }
		void setGradientMode(const int val);

		int getRigidBodyMode() const { return static_cast<int>(m_rbMode); }
		void setRigidBodyMode(const int val);

    bool useRigidContactSolver() const {return m_useRigidContactSolver; }
    bool useRigidGradientManager() const {return m_useRigidGradientManager; }
    bool useReleaseRigidBodyMode() const {return m_releaseRigidBodyMode; }

    Real getFrictionCoeff() const {return m_friction_coeff; }
    Real getBetaScaleFactor() const {return m_beta_scale_factor; }
    Real getRRContactGammaFactor() const {return m_rigid_contact_gamma_factor; }
    Real getRRContactDensityErrorThresh() const {return m_rigid_contact_density_err_thresh; }
    Real getRRContactSupportRadiusFactor() const {return m_rigid_contact_support_radius_factor; }
    unsigned getRRContactSolverMaxIter() const {return m_RR_contact_solver_max_iter; }
    bool useRRContactGradient() const {return m_use_RR_contact_gradient; }
    RigidBodyGradientManager* getRigidBodyGradientManager() const {return getSimulatorBase()->getRigidBodyGradientManager();} 
		...
		bool isDebugRigidContact() { return m_isDebugRigidContact; }
    ...
    // The number of particles in {neighborPointSetIndex} which are neighbors of the particle {index} in {pointSetIndex}
