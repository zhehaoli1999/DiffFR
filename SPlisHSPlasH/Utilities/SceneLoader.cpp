...
void SceneLoader::readScene(const char *fileName, Scene &scene)
{
	...
	if (m_jsonData.find("Configuration") != m_jsonData.end())
	{
		...
		scene.useRigidContactSolver = false;
		readValue(config["useRigidContactSolver"], scene.useRigidContactSolver);
		scene.useRigidGradientManager = false;
		readValue(config["useRigidGradientManager"], scene.useRigidGradientManager);
		scene.useReleaseRigidBodyMode = false;
		readValue(config["useReleaseRigidBodyMode"], scene.useReleaseRigidBodyMode);

		scene.rigidContactGamma = 0.7;
		readValue(config["rigidContactGamma"], scene.rigidContactGamma);

		scene.rigidContactBeta = 1.0;
		readValue(config["rigidContactBeta"], scene.rigidContactBeta);

		scene.rigidContactSupportRadiusFactor = 4.0;
		readValue(config["rigidContactSupportRadiusFactor"], scene.rigidContactSupportRadiusFactor);

		scene.rigidContactFrictionCoeff = 0.0;
		readValue(config["rigidContactFrictionCoeff"], scene.rigidContactFrictionCoeff);
    ...
    scene.targetTime = 0.8; 
    readValue(config["targetTime"], scene.targetTime);
    scene.uniformAccelerateRBTime = 0.0; 
    readValue(config["uniformAccelerateRBTime"], scene.uniformAccelerateRBTime);

   scene.gradientMode = 1; // incomplete gradient 
   readValue(config["gradientMode"], scene.gradientMode);
	...
	if (m_jsonData.find("RigidBodies") != m_jsonData.end())
	{
		...
		for (auto& boundaryModel : boundaryModels)
		{
			...
			if (bMesh || bSamples)
			{
				...
				data->density = 1000.0;
				readValue(boundaryModel["density"], data->density);
				...
				//data->isConsiderRRContact = true;
				//readValue<bool>(boundaryModel["isConsiderRRContact"], data->isConsiderRRContact);
				...
				data->target_x = Eigen::Matrix<Real, 3, 1, Eigen::DontAlign>(0.8, 2.0, 0.);
				readVector(boundaryModel["targetX"], data->target_x);

				data->target_angle_in_degree = Eigen::Matrix<Real, 3, 1, Eigen::DontAlign>(0., 0., 0.);
				readVector(boundaryModel["targetAngleInDegree"], data->target_angle_in_degree);

				data->init_velocity = Eigen::Matrix<Real, 3, 1, Eigen::DontAlign>(0., 0., 0.);
				readVector(boundaryModel["initVelocity"], data->init_velocity);

				data->init_angular_velocity = Eigen::Matrix<Real, 3, 1, Eigen::DontAlign>(0., 0., 0.);
				readVector(boundaryModel["initAngularVelocity"], data->init_angular_velocity);

				data->showTargetMode = 0;
				readValue(boundaryModel["showTargetMode"], data->showTargetMode);

				data->targetScale = Vector3r::Ones();
				readVector(boundaryModel["targetScale"], data->targetScale);
	...
	if (m_jsonData.find("ArticulatedSystems") != m_jsonData.end())
	{
		nlohmann::json artSystems = m_jsonData["ArticulatedSystems"];
		for (auto &system : artSystems)
		{
			ArticulatedSystemData *systemData = new ArticulatedSystemData();
			systemData->rigidBodyIndices = system["rigidIndices"].get<std::vector<unsigned int>>();
			for (auto &joint : system["joints"])
			{
				ArticulatedSystemData::JointData data;
				data.rigidIndex1 = 0;
				readValue(joint["rigidIndex1"], data.rigidIndex1);
				data.rigidIndex2 = 1;
				readValue(joint["rigidIndex2"], data.rigidIndex2);
				data.jointPoint = Vector3r(0., 0., 0.);
				readVector(joint["jointPoint"], data.jointPoint);
				if (joint.find("jointRelativePosition") != joint.end())
				{
					Vector3r Pos;
					readVector(joint["jointRelativePosition"], Pos);
					auto &rigidBodyModel = scene.boundaryModels[systemData->rigidBodyIndices[data.rigidIndex2]];
					// relative position -> real position
					// only for unit box model
					for (unsigned int i = 0; i < 3; i++)
					{
						Pos[i] *= rigidBodyModel->scale[i] * 0.5;
					}
					Pos = rigidBodyModel->rotation * Pos;
					for (unsigned int i = 0; i < 3; i++)
					{
						Pos[i] += rigidBodyModel->translation[i];
					}
					data.jointPoint = Pos;
				}
        data.isActuator = false;
        readValue(joint["isActuator"], data.isActuator);
				systemData->joints.push_back(data);
			}
			systemData->inv_stiffness = 0.;
			readValue(system["invStiffness"], systemData->inv_stiffness);
			if (!systemData->joints.empty())
			{
				scene.articulatedSystems.push_back(systemData);
			}
			else
			{
				delete systemData;
	...
	if (m_jsonData.find("FluidBlocks") != m_jsonData.end())
	{
      ...
      //if (readVector(fluidBlock["rotationAxis"], axis) &&
        //readValue<Real>(fluidBlock["rotationAngle"], angle))
