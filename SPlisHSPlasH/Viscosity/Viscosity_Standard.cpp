...
Viscosity_Standard::Viscosity_Standard(FluidModel *model) : ViscosityBase(model)
{
...
// TODO: we currently comment this since this reports access error
//
// #ifdef USE_AVX
				...
				if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012)
				{
					for (unsigned int pid = nFluids; pid < sim->numberOfPointSets(); pid++)
					{
						...
						for (unsigned int j = 0; j < sim->numberOfNeighbors(fluidModelIndex, pid, i); j++)
						{
							...
							{
								const Real factor = d * m_boundaryViscosity * density0 * bm_neighbor->getVolume(neighborIndex);
								auto n = (xixj.squaredNorm() + 0.01 * h2);
								const Real tmp1 = (vi - vj).dot(xixj) / n;
								const Vector3r tmp2 = tmp1 * sim->gradW(xi - xj);

								// TODO: check the transpose
								Matrix3r grad1 = -(Real)1. / density_i * tmp2 * (-sim->gradW(xi - xj)).transpose(); // grad density_i

								// grad xixj
								Matrix3r grad2 = (Real)1. / n * sim->gradW(xi - xj) * -(vi - vj).transpose();

								// grad xixj.squaredNorm
								Matrix3r grad3 = -(vi - vj).dot(xixj) / (n * n) * (Real)2. * sim->gradW(xi - xj) * -xixj.transpose();
								Matrix3r grad4 = tmp1 * -sim->gradGradW(xi - xj);
								Matrix3r grad_a_to_xj = factor / density_i * (grad1 + grad2 + grad3 + grad4);
								// if(j==0)
								//{
								// LOG_INFO << YellowHead() << "[viscosity] grad1 = " << grad1.transpose()<< YellowTail();
								// LOG_INFO << YellowHead() << "[viscosity] grad2 = " << grad2.transpose()<< YellowTail();
								// LOG_INFO << YellowHead() << "[viscosity] grad3 = " << grad3.transpose()<< YellowTail();
								// LOG_INFO << YellowHead() << "[viscosity] grad4 = " << grad4.transpose()<< YellowTail();
								// LOG_INFO << YellowHead() << "[viscosity] grad_a_to_xj = " << grad_a_to_xj.transpose()<< YellowTail();
								//}

								// pure gradient to v_j
								auto grad_a_to_vj = -factor / density_i / (xixj.squaredNorm() + 0.01 * h2) * sim->gradW(xi - xj) * xixj.transpose();
								// if(j==0)
								// LOG_INFO << YellowHead() << "[viscosity] grad_a_to_vj = " << grad_a_to_vj.transpose()<< YellowTail();

								auto dt = TimeManager::getCurrent()->getTimeStepSize();
								auto grad_force_to_vj = -m_model->getMass(i) * (grad_a_to_vj + dt * grad_a_to_xj);
								// if(j==0)
								// LOG_INFO << YellowHead() << "[viscosity] grad_force_to_vj = " << grad_force_to_vj.transpose()<< YellowTail();
								bm_neighbor->get_grad_force_to_v(neighborIndex) += grad_force_to_vj;
							}
