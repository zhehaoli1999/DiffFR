...
void TimeStepDFSPH::warmstartPressureSolve(const unsigned int fluidModelIndex)
{
	...
	#pragma omp parallel default(shared)
	{
        ...
        max(m_simulationData.getKappa(fluidModelIndex, i),
            static_cast<Real>(-0.00025)) * invH2;
			else
				m_simulationData.getKappa(fluidModelIndex, i) = 0.0;
		}
...
void TimeStepDFSPH::warmstartDivergenceSolve(const unsigned int fluidModelIndex)
{
	...
	#pragma omp parallel default(shared)
	{
        ...
        m_simulationData.getKappaV(fluidModelIndex, i) =
        static_cast<Real>(0.5) *
        max(m_simulationData.getKappaV(fluidModelIndex, i),
            static_cast<Real>(-0.5)) * invH;
			else
				m_simulationData.getKappaV(fluidModelIndex, i) = 0.0;
		}
...
//void TimeStepDFSPH::pressureSolveIterationMLS(const unsigned int fluidModelIndex, Real &avg_density_err)
//{
	//Simulation *sim = Simulation::getCurrent();
	//FluidModel *model = sim->getFluidModel(fluidModelIndex);
	//const Real density0 = model->getDensity0();
  ...
  //// compute boundary pressure using MLS 
  //if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012)
  //{
