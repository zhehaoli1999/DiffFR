...
int Simulation::IS_DEBUG = -1;
int Simulation::IS_DEBUG_RIGID_CONTACT = -1;
int Simulation::IS_RELEASE_RIGID_BODY_MODE = -1;
int Simulation::USE_RIGID_CONTACT_SOLVER = -1;
int Simulation::USE_RIGID_GRADIENT_MANAGER = -1;
int Simulation::FRICTION_COEFF = -1;
int Simulation::BETA_SCALE_FACTOR = -1;
int Simulation::RR_CONTACT_GAMMA_FACTOR = -1;
int Simulation::RR_CONTACT_DENSITY_ERR_THRESH = -1;
int Simulation::RR_CONTACT_SOLVER_MAX_ITER = -1;
int Simulation::USE_RR_CONTACT_GRADIENT = -1;
...
int Simulation::GRADIENT_MODE = -1;
int Simulation::ENUM_GRADIENT_COMPLETE= -1;
int Simulation::ENUM_GRADIENT_INCOMPLETE= -1;
int Simulation::ENUM_GRADIENT_RIGIDONLY= -1;
int Simulation::RIGIDBODY_MODE = -1;
int Simulation::ENUM_RB_WITH_GYROSCOPIC= -1;
int Simulation::ENUM_RB_NO_GYROSCOPIC= -1;
...
int Simulation::ENUM_SIMULATION_DiffDFSPH = -1;
...
Simulation::Simulation ()
{
  ...
  //m_gradientMode = GradientMode::Incomplete;
  m_gradientMode = static_cast<GradientMode>(SceneConfiguration::getCurrent()->getScene().gradientMode);
  m_rbMode = RigidBodyMode::WithGyroscopic;
  m_simulationMethodChanged = NULL;

  m_simulationIsInitialized = false;
  m_sim2D = false;
  m_enableZSort = true;
  m_releaseRigidBodyMode = SceneConfiguration::getCurrent()->getScene().useReleaseRigidBodyMode;

  m_useRigidContactSolver =SceneConfiguration::getCurrent()->getScene().useRigidContactSolver;
  m_useRigidGradientManager =SceneConfiguration::getCurrent()->getScene().useRigidGradientManager;

  m_rigid_contact_gamma_factor =SceneConfiguration::getCurrent()->getScene().rigidContactGamma;
  m_friction_coeff = SceneConfiguration::getCurrent()->getScene().rigidContactFrictionCoeff;
  m_beta_scale_factor = SceneConfiguration::getCurrent()->getScene().rigidContactBeta;
  m_rigid_contact_support_radius_factor = SceneConfiguration::getCurrent()->getScene().rigidContactSupportRadiusFactor;
  m_RR_contact_solver_max_iter = 50;
  m_rigid_contact_density_err_thresh = 0.1 / 100.; // 0.1%
  m_use_RR_contact_gradient = true;
  ...
  m_isDebug = false;
  m_isDebugRigidContact = false;
}

Simulation::Simulation(SimulatorBase* simulatorBase)
:Simulation()
{
  m_simulatorBase = simulatorBase;
}
...
Simulation* Simulation::getCurrentWithBase(SimulatorBase* simBase)
{
  if (current == nullptr)
  {
    current = new Simulation (simBase);
  }
  return current;
}
...
void Simulation::initParameters()
{
  ...
  IS_DEBUG = createBoolParameter("is debug", "is debug", &m_isDebug);
  setGroup(IS_DEBUG, "Simulation");
  IS_DEBUG_RIGID_CONTACT= createBoolParameter("is debug rigid contact", "is debug rigid contact", &m_isDebugRigidContact);
  setGroup(IS_DEBUG_RIGID_CONTACT, "Simulation");

  USE_RIGID_CONTACT_SOLVER = createBoolParameter("use rigid contact solver", "use rigid contact solver", &m_useRigidContactSolver);
  setGroup(USE_RIGID_CONTACT_SOLVER, "Simulation");
  USE_RIGID_GRADIENT_MANAGER = createBoolParameter("use rigidbody gradient manager", "use rigidbody gradient manager", &m_useRigidGradientManager);
  setGroup(USE_RIGID_GRADIENT_MANAGER, "Simulation");

  FRICTION_COEFF = createNumericParameter("friction_coeff", "friction_coeff", &m_friction_coeff);
  setGroup(FRICTION_COEFF, "Simulation");
  BETA_SCALE_FACTOR = createNumericParameter("beta factor", "beta factor", &m_beta_scale_factor);
  setGroup(BETA_SCALE_FACTOR, "Simulation");
  RR_CONTACT_DENSITY_ERR_THRESH = createNumericParameter("RR contact thresh", "RR contact thresh", &m_rigid_contact_density_err_thresh);
  setGroup(RR_CONTACT_DENSITY_ERR_THRESH, "Simulation");
  RR_CONTACT_GAMMA_FACTOR = createNumericParameter("RR contact gamma", "RR contact gamma", &m_rigid_contact_gamma_factor);
  setGroup(RR_CONTACT_GAMMA_FACTOR, "Simulation");
  RR_CONTACT_SOLVER_MAX_ITER = createNumericParameter("RR contact max iter", "RR contact max iter", &m_RR_contact_solver_max_iter);
  setGroup(RR_CONTACT_SOLVER_MAX_ITER, "Simulation");
  USE_RR_CONTACT_GRADIENT = createBoolParameter("use RR contact gradient", "use RR contact gradient", &m_use_RR_contact_gradient);
  setGroup(USE_RR_CONTACT_GRADIENT, "Simulation");
  IS_RELEASE_RIGID_BODY_MODE = createBoolParameter("release rigid body mode", "release rigid body mode", &m_releaseRigidBodyMode);
  setGroup(IS_RELEASE_RIGID_BODY_MODE, "Simulation");
  ...
  enumParam->addEnumValue("DiffDFSPH", ENUM_SIMULATION_DiffDFSPH);
  enumParam->addEnumValue("Projective Fluids", ENUM_SIMULATION_PF);
  enumParam->addEnumValue("ICSPH", ENUM_SIMULATION_ICSPH);

  ParameterBase::GetFunc<int> getGradientModeFct = std::bind(&Simulation::getGradientMode, this);
  ParameterBase::SetFunc<int> setGradientModeFct = std::bind(&Simulation::setGradientMode, this, std::placeholders::_1);
  GRADIENT_MODE = createEnumParameter("gradientMode", "gradient mode", getGradientModeFct, setGradientModeFct);
  setGroup(GRADIENT_MODE, "Simulation");
  enumParam = static_cast<EnumParameter*>(getParameter(GRADIENT_MODE));
  enumParam->addEnumValue("complete", ENUM_GRADIENT_COMPLETE);
  enumParam->addEnumValue("incomplete", ENUM_GRADIENT_INCOMPLETE);
  enumParam->addEnumValue("rigid only", ENUM_GRADIENT_RIGIDONLY);

  ParameterBase::GetFunc<int> getRigidBodyModeFct = std::bind(&Simulation::getRigidBodyMode, this);
  ParameterBase::SetFunc<int> setRigidBodyModeFct = std::bind(&Simulation::setRigidBodyMode, this, std::placeholders::_1);
  RIGIDBODY_MODE = createEnumParameter("rigibody mode", "rigidbody mode", getRigidBodyModeFct, setRigidBodyModeFct);
  setGroup(RIGIDBODY_MODE, "Simulation");
  enumParam = static_cast<EnumParameter*>(getParameter(RIGIDBODY_MODE));
  enumParam->addEnumValue("with gyroscopic", ENUM_RB_WITH_GYROSCOPIC);
  enumParam->addEnumValue("no gyroscopic", ENUM_RB_NO_GYROSCOPIC);
...
void Simulation::setGradKernel(int val)
{
  ...
  if (!m_sim2D)
  {
    ...
    if (m_gradKernelMethod == 0)
    {
      ...
      m_gradKernelFct_with_h = CubicKernel::gradW_with_h;
      m_gradGradKernelFct = CubicKernel::gradGradW;
      m_gradGradKernelFct_with_h = CubicKernel::gradGradW_with_h;
    }
  else if (m_gradKernelMethod == 1)
    {
      m_gradKernelFct = WendlandQuinticC2Kernel::gradW;
      m_gradGradKernelFct = WendlandQuinticC2Kernel::gradGradW;
    }
  else if (m_gradKernelMethod == 2)
    {
      m_gradKernelFct = Poly6Kernel::gradW;
      m_gradGradKernelFct = Poly6Kernel::gradGradW;
    }
  else if (m_gradKernelMethod == 3)
    {
      m_gradKernelFct = SpikyKernel::gradW;
      m_gradGradKernelFct = SpikyKernel::gradGradW;
    }
  else if (m_gradKernelMethod == 4)
    {
      m_gradKernelFct = Simulation::PrecomputedCubicKernel::gradW;
      m_gradGradKernelFct = Simulation::PrecomputedCubicKernel::gradGradW;
    }
  }
else
  {
    ...
    if (m_gradKernelMethod == 0)
    {
      ...
      m_gradGradKernelFct = CubicKernel2D::gradGradW;
      m_gradKernelFct_with_h = CubicKernel2D::gradW_with_h;
      m_gradGradKernelFct_with_h = CubicKernel2D::gradGradW_with_h;
    }
  else if (m_gradKernelMethod == 1)
    {
      m_gradKernelFct = WendlandQuinticC2Kernel2D::gradW;
      m_gradGradKernelFct = WendlandQuinticC2Kernel2D::gradGradW;
    }
  }
}
...
void Simulation::setKernel(int val)
{
  ...
  if (!m_sim2D)
  {
    ...
    if (m_kernelMethod == 0)
    {
      ...
      m_kernelFct_with_h = CubicKernel::W_with_h;
    }
...
else
  {
    ...
    if (m_kernelMethod == 0)
    {
      ...
      m_kernelFct_with_h = CubicKernel2D::W_with_h;
    }
  else if (m_kernelMethod == 1)
    {
...
void Simulation::setGradientMode(const int val)
{
  m_gradientMode = static_cast<GradientMode>(val);
}
void Simulation::setRigidBodyMode(const int val)
{
  m_rbMode = static_cast<RigidBodyMode>(val);
}
...
else if (method == SimulationMethods::DiffDFSPH)
  {
    m_timeStep = new TimeStepDiffDFSPH();
    m_timeStep->init();
    setValue(Simulation::KERNEL_METHOD, Simulation::ENUM_KERNEL_CUBIC);
    setValue(Simulation::GRAD_KERNEL_METHOD, Simulation::ENUM_KERNEL_CUBIC);
  }
...
void Simulation::performNeighborhoodSearchSort()
{
  ...
  //if(useRigidContactSolver()) // this step should before bm->performNeighborhoodSearchSort

  if(useRigidContactSolver())
    // FIXME: directly call m_simulatorBase->getBoundarySimulator() cause error, why?
    m_simulatorBase->getBoundarySimulator()->performNeighborhoodSearchSort();
...
void Simulation::updateBoundaryVolume()
{
  ...
  // For MLS intepolation 
  for (unsigned int body = 0; body < numberOfBoundaryModels(); body++)
    {
      //if (getBoundaryModel(body)->getRigidBodyObject()->isDynamic() || getBoundaryModel(body)->getRigidBodyObject()->isAnimated())
      //{
        for (unsigned int j = 0; j < numberOfFluidModels(); j++)
           m_neighborhoodSearch->set_active(body+nFluids, j, true); // dynamic boundary particles should find fluid particle neighbors for MLS
      //}
  }
