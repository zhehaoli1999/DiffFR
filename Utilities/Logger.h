...
// 			else 
	...
	public:
		...
		void noConsoleWrite(const LogLevel level, const std::string &str)
		{
			if (!m_active)
				return;
			for (unsigned int i = 0; i < m_sinks.size(); i++)
			{
				if (dynamic_cast<ConsoleSink *>(m_sinks[i].get()))
					continue;
				m_sinks[i]->write(level, str);
			}
		}
	...
	class NoConsoleLogStream
	{
	public:
		NoConsoleLogStream(Logger *logger, const LogLevel level) : m_logger(logger), m_level(level) {}

		template <typename T>
		NoConsoleLogStream& operator<<(T const & value)
		{
			m_buffer << value;
			return *this;
		}

		~NoConsoleLogStream() { m_logger->noConsoleWrite(m_level, m_buffer.str()); }
	...
	#define NO_CONSOLE_LOG_DEBUG Utilities::NoConsoleLogStream(&Utilities::logger, Utilities::LogLevel::DEBUG)
	#define NO_CONSOLE_LOG_INFO Utilities::NoConsoleLogStream(&Utilities::logger, Utilities::LogLevel::INFO)
	#define NO_CONSOLE_LOG_WARN Utilities::NoConsoleLogStream(&Utilities::logger, Utilities::LogLevel::WARN)
	#define NO_CONSOLE_LOG_ERR Utilities::NoConsoleLogStream(&Utilities::logger, Utilities::LogLevel::ERR)
}
