...
#define get_delta_time(id, code)                                                                        \
{                                                                                                     \
  auto t0 = std::chrono::high_resolution_clock::now();                                                \
  code \
  auto t1 = std::chrono::high_resolution_clock::now();                                           \
  auto delta_t = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count() / 1000.0;  \
  LOG_INFO << Utilities::YellowHead() << "delta t [" << id << "] = " << delta_t << " ms" << Utilities::YellowHead(); \
}
