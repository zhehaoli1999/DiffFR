...
int SimulatorBase::RENDER_TARGETS = -1;
int SimulatorBase::ENUM_TARGETS_NONE = -1;
int SimulatorBase::ENUM_TARGETS_POSITIONS = -1;
int SimulatorBase::ENUM_TARGETS_ANGLES = -1;
int SimulatorBase::ENUM_TARGETS_POSITIONS_AND_ANGLES = -1;
int SimulatorBase::ENUM_TARGETS_MESH = -1;
int SimulatorBase::ENUM_TARGETS_ALL = -1;
...
SimulatorBase::SimulatorBase()
{
	...
	m_rigidBodyGradientManager = nullptr;
	m_gui = nullptr;
  ...
  m_renderTargets = 0;  
	...
	m_useGUI = true;
	m_loadFluidPos = false;
	m_loadFluidPosAndVel = false;
...
void SimulatorBase::initParameters() {
  ...
  PAUSE_AT =
      createNumericParameter("pauseAt", "Pause simulation at", &m_pauseAt);
  setGroup(PAUSE_AT, "General");
  setDescription(PAUSE_AT, "Pause simulation at the given time. When the value "
                           "is negative, the simulation is not paused.");

  STOP_AT = createNumericParameter("stopAt", "Stop simulation at", &m_stopAt);
  setGroup(STOP_AT, "General");
  setDescription(STOP_AT, "Stop simulation at the given time. When the value "
                          "is negative, the simulation is not stopped.");

  NUM_STEPS_PER_RENDER = createNumericParameter(
      "numberOfStepsPerRenderUpdate", "# time steps / update",
  ...
  static_cast<NumericParameter<unsigned int> *>(
      getParameter(NUM_STEPS_PER_RENDER))
      ->setMinValue(1);

  RENDER_WALLS =
      createEnumParameter("renderWalls", "Render walls", &m_renderWalls);
  setGroup(RENDER_WALLS, "Visualization");
  setDescription(RENDER_WALLS, "Make walls visible/invisible.");
  EnumParameter *enumParam =
      static_cast<EnumParameter *>(getParameter(RENDER_WALLS));
  enumParam->addEnumValue("None", ENUM_WALLS_NONE);
  enumParam->addEnumValue("Particles (all)", ENUM_WALLS_PARTICLES_ALL);
  enumParam->addEnumValue("Particles (no walls)",
                          ENUM_WALLS_PARTICLES_NO_WALLS);
  ...
  RENDER_TARGETS =
      createEnumParameter("renderTargets", "Render targets", &m_renderTargets);
  setGroup(RENDER_TARGETS, "Visualization");
  setDescription(RENDER_TARGETS, "Make targets visible/invisible.");
  EnumParameter *enumParamTarget =
      static_cast<EnumParameter *>(getParameter(RENDER_TARGETS));
  enumParamTarget->addEnumValue("None", ENUM_TARGETS_NONE);
  enumParamTarget->addEnumValue("Position", ENUM_TARGETS_POSITIONS);
  enumParamTarget->addEnumValue("Angle", ENUM_TARGETS_ANGLES);
  enumParamTarget->addEnumValue("Position & Angle",
                                ENUM_TARGETS_POSITIONS_AND_ANGLES);
  enumParamTarget->addEnumValue("Mesh", ENUM_TARGETS_MESH);
  enumParamTarget->addEnumValue("All", ENUM_TARGETS_ALL);

  DATA_EXPORT_FPS =
      createNumericParameter("dataExportFPS", "Export FPS", &m_framesPerSecond);
  setGroup(DATA_EXPORT_FPS, "Export");
  setDescription(DATA_EXPORT_FPS,
                 "Frame rate of partio, vtk and rigid body export.");

  STATE_EXPORT = createBoolParameter(
      "enableStateExport", "Simulation state export", &m_enableStateExport);
  setGroup(STATE_EXPORT, "Export");
  setDescription(STATE_EXPORT,
                 "Enable/disable export of complete simulation state.");

  STATE_EXPORT_FPS = createNumericParameter(
      "stateExportFPS", "State export FPS", &m_framesPerSecondState);
  setGroup(STATE_EXPORT_FPS, "Export");
  setDescription(STATE_EXPORT_FPS, "Frame rate of simulation state export.");

  PARTICLE_EXPORT_ATTRIBUTES = createStringParameter(
      "particleAttributes", "Export attributes", &m_particleAttributes);
  getParameter(PARTICLE_EXPORT_ATTRIBUTES)->setReadOnly(true);
  setGroup(PARTICLE_EXPORT_ATTRIBUTES, "Export");
  setDescription(PARTICLE_EXPORT_ATTRIBUTES,
                 "Attributes that are exported in the partio files (except id "
                 "and position).");

  ASYNC_EXPORT = createBoolParameter("enableAsyncExport", "Asynchronous export",
                                     &m_enableAsyncExport);
  setGroup(ASYNC_EXPORT, "Export");
  setDescription(
      ASYNC_EXPORT,
      "Enable/disable asynchronous export of data. The total performance is "
      "faster but disable it when measuring the timings of simulation "
  ...
  for (size_t i = 0; i < m_particleExporters.size(); i++) {
    ...
    setDescription(m_particleExporters[i].m_id,
                   m_particleExporters[i].m_description);
  }
  ...
  EXPORT_OBJECT_SPLITTING = createBoolParameter(
      "enableObjectSplitting", "Split SPH objects", &m_enableObjectSplitting);
  setGroup(EXPORT_OBJECT_SPLITTING, "Export");
  setDescription(EXPORT_OBJECT_SPLITTING,
                 "Enable/disable an export of the SPH particles with an "
                 "indiviual file per object.");
...
void SimulatorBase::init(int argc, char **argv, const std::string &windowName)
{
	...
	try
	{
		...
		options.add_options()
			...
			("load-fluid-pos", "only load fluid positions from state file") 
			("load-fluid-pos-and-vel", "load fluid positions and velocities from state file") 
		...
		if (result.count("load-fluid-pos"))
		{
			setLoadFluidPos(true);
		}

		if (result.count("load-fluid-pos-and-vel"))
		{
			setLoadFluidPosAndVel(true);
		}
    ...
    m_boundarySimulator = new RigidBody3dBoundarySimulator(this);
	}
	else
	{
		LOG_INFO << "Initialize 2d dynamic boudary simulation";
		m_boundarySimulator = new Dynamic2dBoundarySimulator(this);
	}
	initExporters();

  // init rigidboy gradient manager 
  m_rigidBodyGradientManager = std::unique_ptr<RigidBodyGradientManager>(new RigidBodyGradientManager()); 
}
...
void SimulatorBase::initSimulation()
{
	...
	Simulation *sim = Simulation::getCurrentWithBase(this);
...
void SimulatorBase::deferredInit()
{
    ...
    unsigned int nDynamicBoundaryParticles = 0; 
		for (unsigned int i = 0; i < sim->numberOfBoundaryModels(); i++)
    {
      unsigned int n = static_cast<BoundaryModel_Akinci2012*>(sim->getBoundaryModel(i))->numberOfParticles();
			nBoundaryParticles += n;  
      if(sim->getBoundaryModel(i)->getRigidBodyObject()->isDynamic())
      {
          nDynamicBoundaryParticles += n; 
          LOG_INFO << "mass of dynamic rigid body: " << sim->getBoundaryModel(i)->getRigidBodyObject()->getMass();
      }
    }
    

		LOG_INFO << "Number of boundary particles: " << nBoundaryParticles;
		LOG_INFO << "Number of dynamic boundary particles: " << nDynamicBoundaryParticles;
	}
  ...
  // 
  m_rigidBodyGradientManager->deferredInit(); 
}
...
void SimulatorBase::reset()
{
  ...
  m_rigidBodyGradientManager->reset();
...
void SimulatorBase::runFixedTimeSteps(unsigned int n)
{
...
{
	//get_delta_time("overall SimulatorBase::timeStep", 
	if (m_updateGUI)
	{
		m_gui->initSimulationParameterGUI();
		m_updateGUI = false;
  ...
  // TODO: add beforeTimeStepCB here 
  if (m_timeStepCallBefore)
    m_timeStepCallBefore();

	const unsigned int numSteps = getValue<unsigned int>(SimulatorBase::NUM_STEPS_PER_RENDER);
	for (unsigned int i = 0; i < numSteps; i++)
	{
		//get_delta_time("SimStep",
		Simulation::getCurrent()->getTimeStep()->step();
   //);

    if(sim->useRigidGradientManager() && sim->numberOfFluidModels() > 0)
      m_rigidBodyGradientManager->after_Fluid_Rigid_coupling_step(); 

		//get_delta_time("m_boundarySimulator->timeStep",
		m_boundarySimulator->velocityTimeStep(); 
    //)
    
    if(sim->useRigidGradientManager())
      m_rigidBodyGradientManager->after_Rigid_Rigid_coupling_step(); 

		m_boundarySimulator->positionTimeStep(); 
...
void SimulatorBase::checkLoadState()
{
  if(m_loadFluidPos)
  {
    LOG_INFO << "[Load state] Load fluid positions from state file " << getStateFile();
    loadFluidParticlePositions(getStateFile());
    loadDynamicRigidBodyParticlePositionsAndVelocities(getStateFile());
    Simulation::getCurrent()->updateBoundaryVolume(); 
  }
  else if(m_loadFluidPosAndVel)
  {
    LOG_INFO << "[Load state] Load fluid positions and velocity from state file " << getStateFile();
    loadFluidParticlePositionsAndVelocities(getStateFile());
    loadDynamicRigidBodyParticlePositionsAndVelocities(getStateFile());
    Simulation::getCurrent()->updateBoundaryVolume(); 
  }
  else
...
void SimulatorBase::initSimulationWithDeferredInit()
{
  initSimulation();
  // ---------------------------------------------------------------
	deferredInit(); 
...
void SimulatorBase::runNewTrajectory()
{
  auto timestep = dynamic_cast<TimeStepDiffDFSPH*>(Simulation::getCurrent()->getTimeStep()); 
  if(timestep == nullptr)
  {
      std::cout << "runNewTrajectory does not support timestep other than TimeStepDiffDFSPH!" << std::endl;
      return;
  }
  timestep->set_in_new_trajectory(true);
  reset();
  while(1)
  {
    this->singleTimeStep();  
    if (timestep->isTrajectoryFinished())
        break;
  }
  timestep->set_in_new_trajectory(false);
}


// Used in controller task 
void SimulatorBase::forwardFixedStepsWithState(const unsigned start_step, const unsigned fixed_steps )
{
	Simulation *sim = Simulation::getCurrent();
  //Simulation 
  //First, load statefile (with number of step in its name) of start_step
	auto stateFilePath = FileSystem::normalizePath(m_outputPath + "/state");
	auto stepStateFileName = FileSystem::normalizePath(stateFilePath + "/state_step" + std::to_string(start_step));
  // TODO: check if this state file exists 
  this->loadState(stepStateFileName);
  // TODO: need to set needActorInput in timestep  
  // Simulation::getCurrent()->getTimeStep()->setNeedActorInput();
...
// Used in controller task 
void SimulatorBase::forwardFixedSteps(const unsigned fixed_steps )
{
	Simulation *sim = Simulation::getCurrent();
...
std::string SPH::SimulatorBase::saveState(const std::string& stateFileDir)
{
	std::string stateFilePath; 
	std::string exportFileName;
	const Real time = TimeManager::getCurrent()->getTime();
	const std::string timeStr = StringTools::real2String(time);
  const std::string frameIndex = std::to_string(m_stepCounter);

	if (stateFileDir == "")
	{
		stateFilePath = FileSystem::normalizePath(m_outputPath + "/state");
    // To load bgeo into houdini as a sequence, we need this frameIndex here 
    //exportFileName= FileSystem::normalizePath(stateFilePath + "/0." + frameIndex + "_state_" + timeStr);
    //exportFileName= FileSystem::normalizePath(stateFilePath + "/" + frameIndex + "_state");
    exportFileName = FileSystem::normalizePath(stateFilePath + "/state_" + frameIndex);
	}
	else
	{
		...
		exportFileName = FileSystem::normalizePath(stateFileDir + "/state_" + frameIndex);
	}
	...
	for (unsigned int i = 0; i < sim->numberOfFluidModels(); i++)
	{
		...
		fileName = fileName + "_" + model->getId(); // +"_" + std::to_string(m_frameCounter - 1);
	...
	for (unsigned int i = 0; i < sim->numberOfBoundaryModels(); i++)
	{
		...
		if (bm->getRigidBodyObject()->isDynamic() || bm->getRigidBodyObject()->isAnimated())
		{
			...
			std::string fileName = "dynamicRB";
			fileName = fileName + "_" + to_string(i); // + "_" + std::to_string(m_frameCounter-1);

			// Save particle data
			std::string expFileName = FileSystem::normalizePath(exportFileName + "_" + fileName);
			writeBoundaryState( expFileName + ".bgeo", bm);
		}
	}
	binWriter.closeFile();
	LOG_INFO << "Saved state: " << exportFileName + ".bin";

	binWriter.openFile(exportFileName + "_dynamicRB.bin");
	// dynamic bodies
	for (unsigned int i = 0; i < sim->numberOfBoundaryModels(); i++)
	{
		BoundaryModel *bm = sim->getBoundaryModel(i);
	...
	binWriter.closeFile();
	LOG_INFO << "Saved dynamic RB state: " << exportFileName + "_dynamicRB.bin";
...
void SPH::SimulatorBase::loadFluidParticlePositionsAndVelocities(const std::string &stateFile)
{
  // This function is called after deferedinit 
	Simulation *sim = Simulation::getCurrent();
...
void SPH::SimulatorBase::loadFluidParticlePositions(const std::string &stateFile)
{
  // This function is called after deferedinit 
  loadFluidParticlePositionsAndVelocities(stateFile);

  // Clear the velocity of all fluid models
	Simulation *sim = Simulation::getCurrent();
	for (unsigned int i = 0; i < sim->numberOfFluidModels(); i++)
	{
		FluidModel *model = sim->getFluidModel(i);
    model->clearVelocities();
  }
}

void SPH::SimulatorBase::loadDynamicRigidBodyParticlePositionsAndVelocities(const std::string &stateFile)
{
	Simulation *sim = Simulation::getCurrent();
	...
	const std::string importFilePath = FileSystem::getFilePath(stateFile);
	const std::string importFileName = FileSystem::getFileName(stateFile);
  std::string dynamic_rb_state_file = importFilePath + "/" + importFileName + "_dynamicRB.bin";
	if (!binReader.openFile(dynamic_rb_state_file))
  {
    std::cout << "cannot open dynamic rb state file : " << dynamic_rb_state_file << std::endl;
    return;
  }
  ...
  for (unsigned int i = 0; i < sim->numberOfBoundaryModels(); i++)
  {
    ...
    if (bm->getRigidBodyObject()->isDynamic() || bm->getRigidBodyObject()->isAnimated())
    {
      // Note: use "binReader.readMatrix(x)" here is wrong! 
      // the binReader reads the buffer sequentially
      
      // Note: directly load bgeo without set the inner state variable of RigidBodyObject is wrong 
      //std::string fileName = "dynamicRB";
      //fileName = fileName + "_" + std::to_string(i);

      //std::string impFileName = FileSystem::normalizePath(importFilePath + "/" + importFileName + "_" + fileName);
      //readBoundaryState(impFileName + ".bgeo", bm);
...
void SPH::SimulatorBase::loadStateWithRigidExisted(const std::string &stateFile)
{
	Simulation *sim = Simulation::getCurrent();
	const std::string& sceneFile = SceneConfiguration::getCurrent()->getSceneFile();
	string md5Str = FileSystem::getFileMD5(sceneFile);
...
//			LOG_INFO << "Store field: " << field.name;
	...
	Partio::write(fileName.c_str(), particleData);
