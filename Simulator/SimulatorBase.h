...
namespace SPH
{
    ...
    std::unique_ptr<RigidBodyGradientManager> m_rigidBodyGradientManager;
		...
		std::function<void()> m_timeStepCallBefore;
	...
	public:
		...
		static int ENUM_TARGETS_NONE;
		static int ENUM_TARGETS_POSITIONS;
		static int ENUM_TARGETS_ANGLES;
		static int ENUM_TARGETS_POSITIONS_AND_ANGLES;
		static int ENUM_TARGETS_MESH;
		static int ENUM_TARGETS_ALL;
    ...
    void initSimulationWithDeferredInit(); 
    void checkLoadState();
    void forwardFixedStepsWithState(const unsigned int start_step, const unsigned int fixed_num_step);
    void forwardFixedSteps(const unsigned int fixed_num_step);
    void runNewTrajectory();

		void setTimeStepCB(std::function<void()> const& callBackFct) { m_timeStepCB = callBackFct; }
		void setTimeStepCallBefore(std::function<void()> const& callBeforeFct) { m_timeStepCallBefore = callBeforeFct; }
		...
		void runFixedTimeSteps(unsigned int);
		void singleTimeStep();
    std::string saveState(const std::string& stateFileDir = "");
		void loadStateDialog();
		void loadState(const std::string &stateFile);
		void loadStateWithRigidExisted(const std::string &stateFile);
    // Zhehao Li: To only load positions of fluid particles from state file
		void loadFluidParticlePositions(const std::string &stateFile); 
		void loadFluidParticlePositionsAndVelocities(const std::string &stateFile); 
		void loadDynamicRigidBodyParticlePositionsAndVelocities(const std::string &stateFile); 
    ...
    void writeFluidParticlesState(const std::string &fileName, FluidModel *model);
		...
		void setLoadFluidPos(bool val) { m_loadFluidPos = val; }
		void setLoadFluidPosAndVel(bool val) { m_loadFluidPosAndVel= val; }
    ...
    SPH::RigidBodyGradientManager * getRigidBodyGradientManager() const {return m_rigidBodyGradientManager.get();}
		...
		bool isSim2d() const { return m_isSim2d; }
    ...
    void setStateExportPath(std::string newStateExportPath) {m_stateExportPath = newStateExportPath; } 
