...
VectorXr MiniGL::m_diffuseIntensity;
VectorXr MiniGL::m_specularIntensity;
VectorXr MiniGL::m_lightPosition;
...
void MiniGL::drawMesh(const TriangleMesh &mesh, const float * const color)
{
	...
	supplyVertices(0, mesh.getVertices().size(), &vertices[0][0]);
	supplyNormals(2, mesh.getVertexNormals().size(), &vertexNormals[0][0]);
	supplyFaces(3 * nFaces, faces);

	glDrawElements(GL_TRIANGLES, 3 * nFaces, GL_UNSIGNED_INT, (void*)0);
...
void MiniGL::enableShader(const Vector3r& ambientReflectance, const Vector3r& diffuseReflectance, const Vector3r& specularReflectance, const Real shininess, const Real pointSize)
{
	Shader& shader = m_shader;
	shader.begin();
	glUniformMatrix4fv(shader.getUniform("modelview_matrix"), 1, GL_FALSE, &m_modelview_matrix(0,0));
	glUniformMatrix4fv(shader.getUniform("projection_matrix"), 1, GL_FALSE, &m_projection_matrix(0,0));
...
void MiniGL::oneStepInMainLoop()
{
  if(!m_glfw_window)
      std::cout << "!! m_glfw_window is nullptr !!" << std::endl;
  if(!glfwWindowShouldClose(m_glfw_window))
  {
    //glfwPollEvents();

    // This one is related to simulatorBase:timeStep
    //if (idlefunc != nullptr)
    //idlefunc();
    ...
    std::cout << "!! m_glfw_window should close !!" << std::endl;
}

void MiniGL::endMainLoop()
{
	if (destroyfunc != nullptr)
		destroyfunc();
