...
void imguiParameters::createParameterObjectGUI(ParameterObject* paramObj)
{
	...
	for (unsigned int i = 0; i < numParams; i++)
	{
		...
		if ((group == "WCSPH") || (group == "PCISPH") || (group == "PBF") || 
			(group == "IISPH") || (group == "DFSPH") || (group == "DiffDFSPH") || (group == "PF") ||
			(group == "ICSPH") ||
...
void SPH::imguiParameters::addParam(const std::string& pgroup, const std::string& psubgroup, imguiParameter* param)
{
	for (auto group_index = 0; group_index < m_imguiParams.size(); group_index++)
	{
		auto &group = m_imguiParams[group_index];
...
void SPH::imguiParameters::addGroup(const std::string& pgroup, const std::string& psubgroup)
{
	for (auto group_index = 0; group_index < m_imguiParams.size(); group_index++)
	{
		auto group = m_imguiParams[group_index];
